...
func (api *UnifierAPI) handleUnify(e *httpx.Event) error {
	...
	tenantID, tenantErr := tenantguard.TenantScope(requestExplicitTenantID(e), ownerID, "techstack.unifier.unify")
	if tenantErr != nil {
		return tenantErr
	}
	body, tooLarge, err := readRequestBodyLimited(e.Request.Body, maxUnifierRequestBodyBytes)
	...
	workers, err := api.fetchWorkers(e.Request.Context(), tenantID, ownerID)
	if err != nil {
